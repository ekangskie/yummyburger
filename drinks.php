...
if (isset($_POST['add_product'])) {
    ...
    if (!empty($product_image) && move_uploaded_file($product_image_tmp_name, $product_image_folder)) {
        $stmt = $conn->prepare("INSERT INTO `drinks`(name, price, image) VALUES (?, ?, ?)");
...
<body>
    ...
    <a href="products.php" class="option-btn"><i class="fas fa-glass-martini"></i> </a>
<a href="index.php?logout=<?php echo $user_id; ?>" onclick="return confirm('Are you sure you want to logout?');" class="delete-btn"><i class="fas fa-sign-out-alt"></i></a>
        ...
        <!-- Products Section -->
        <div class="products">
            <h1 class="heading">Our Latest Drinks</h1>
            <div class="box-container">
                <?php
                $select_product = $conn->query("SELECT * FROM `drinks`");
        ...
        <div class="shopping-cart">
                    ...
                    ?>
                            <tr>
                             <td><img id="round-img" src="<?php echo htmlspecialchars($fetch_cart['image']); ?>" alt="Product Image"></td>
                                ...
                                <td><a href="drinks.php?remove=<?php echo $fetch_cart['id']; ?>" class="delete-btn" onclick="return confirm('are u sure to remove this?');">Remove</a></td>
                            </tr>
                    <?php
                        }
                    } else {
         ...
         <td><a href="drinks.php?delete_all" onclick="return confirm('delete all from cart?');" class="delete-btn <?php echo ($grand_total > 1)?'':'disabled'; ?>">delete all</a></td>
      </tr>
   </tbody>
   </table>
