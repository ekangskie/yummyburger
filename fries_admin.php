...
if (isset($_POST['edit_dessert'])) {
    ...
    if (!empty($dessert_image) && move_uploaded_file($dessert_image_tmp_name, $dessert_image_folder)) {
        $stmt = $conn->prepare("UPDATE `desserts` SET name = ?, price = ?, image = ? WHERE id = ?");
        $stmt->bind_param("sssi", $dessert_name, $dessert_price, $dessert_image_folder, $dessert_id);
    } else {
        $stmt = $conn->prepare("UPDATE `desserts` SET name = ?, price = ? WHERE id = ?");
...
<body>
 ...
 <div class="flex" style="text-align:center; margin: 20px;">
    ...
    <form method="post" enctype="multipart/form-data">
        <input type="text" name="dessert_name" placeholder="fries Name" required>
        <input type="number" name="dessert_price" placeholder="fries Price" required>
        <input type="file" name="dessert_image" accept="image/*" required>
        <input type="submit" name="add_dessert" value="Add fries">
    </form>
    ...
    <div class="desserts">
        ...
        <div class="box-container">
            <?php
            $select_dessert = $conn->query("SELECT * FROM `desserts`");
            ...
            ?>
                    <form method="post" enctype="multipart/form-data" class="box">
                        ...
                        <input type="submit" value="Update" name="edit_dessert" class="btn">
                    </form>
            <?php
                }
            } else {
